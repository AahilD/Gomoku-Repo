...
public class PVEnvironment extends GameController {
	...
	/**
	 * If you are initializing a Player Versus Environment session, call this method
	 * to setup the game for the first time. (To initialize a Player v Player
	 * session, call the same method in the PvPController instead; do not use the
	 * one in GameController).
	 * 
	 * @param player1name is a string value that represents the username for player
	 *                    1
	 */
	...
	/**
	 * The GUI should call this method to play the move requested by the user. then
	 * it will prompt the environment to make a move immediately afterwards based on
	 * the level setting the user selected for this PVE session.
	 * 
	 * @param x is the (n)th row (starting at 0) of the selected square the user
	 *          requested to place the piece on.
	 * @param y is the (n)th column (starting at 0) of the selected square the user
	 *          requested to place the piece on.
	 */
	...
	/**
	 * This private method will determine which method will be executed to play a
	 * move based on the level selected. This method acts like a switch board.
	 */
	 ...
	 * coordinates. If the environment plays an illegal move it will simply generate
	 * a new random location until it finally plays a succesful move.
	 * 
	 */
	private static void environment_lvl_zero() {
		...
		if (!availableSquareCoordinates.isEmpty()) {
			Square sqr = availableSquareCoordinates.get(r.nextInt(availableSquareCoordinates.size()));
	...
	private static void tryToPlayMove(int x, int y) throws IllegalMove, WinAndLosses {
		playMove(x, y);
		MainGUI.updateBoardSquareButton(x, y, game.getTurnPlayer().getPieceColour());
